...
async function askClaude(channelId, userMessage, imageUrls = []) {
  ...
  // Use vision model for images, fast text model otherwise
  ...
  // Store text-only in history so future messages stay compatible
...
client.on('messageCreate', async (message) => {
  ...
  if (aiConversations.has(message.channel.id) && !message.content.startsWith('.')) {
    ...
    try {
      ...
      // Collect any image attachments from the message
