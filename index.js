...
// ══════════════════════════════════════════════════════════════════════════════
// GEOMETRY DASH MINI-GAME (INFINITE - NO WIN)
// ══════════════════════════════════════════════════════════════════════════════
// Layout: 3 rows
//   Row 0 (top)    = jump row — player appears here when jumping
//   Row 1 (middle) = ground row — player stands here normally
//   Row 2 (floor)  = solid floor ⬛ decorative
// Spikes sit on the ground row. Player jumps UP to row 0 to avoid them.

const TRACK_LENGTH = 12;
const PLAYER_POS = 1;

function buildGDFrame(jumping, spikes, gameOver) {
  const topRow = [];    // jump lane
  const midRow = [];    // ground lane
  const floorRow = [];  // solid floor

  for (let x = 0; x < TRACK_LENGTH; x++) {
    const hasSpike = spikes.some(s => s.x === x);
    const isPlayer = x === PLAYER_POS;

    // Top row: player only appears here when jumping
    topRow.push(isPlayer && jumping ? '🟦' : '⬛');

    // Mid row: player on ground, or spike, or empty ground
    if (isPlayer && !jumping) {
      midRow.push('🟦');
    } else if (hasSpike) {
      midRow.push('🔺');
...
async function startGDGame(channel, existingMessage = null) {
  ...
  // 1100ms per tick gives enough reaction time
  state.interval = setInterval(() => tickGD(msg, state), 1100);
}
...
async function tickGD(msg, state) {
  ...
  // Spawn new spike — always keep 2 on track, min 7 gap
  while (state.spikes.length < 2) {
    const maxX = state.spikes.length > 0
      ? Math.max(...state.spikes.map(s => s.x))
      : TRACK_LENGTH;
    state.spikes.push({ x: maxX + 7 + Math.floor(Math.random() * 4) });
  }

  // Jump arc: 1 frame up, 1 frame up, then land
  if (state.jumping) {
    state.jumpFrames++;
  ...
  // Collision: spike at player pos AND player on ground
