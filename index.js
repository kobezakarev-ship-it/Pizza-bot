...
// ── AI VOICE CHAT CHANNELS ────────────────────────────────────────────────────
// userId → channelId so each user can only have one AI voice chat open at a time
const aiVoiceChannels = new Map();
const aiVoiceConversations = new Map();
...
// GROQ AI HELPER (text)
// ══════════════════════════════════════════════════════════════════════════════
async function askClaude(channelId, userMessage, imageUrls = []) {
  ...
  history[history.length - 1] = { role: 'user', content: userMessage || 'What is in this image?' };
...
// ══════════════════════════════════════════════════════════════════════════════
// GROQ VOICE HELPERS
// Transcribe audio with Groq Whisper, then reply with TTS voice message
// ══════════════════════════════════════════════════════════════════════════════

/**
 * Transcribes an audio file (any common format) using Groq Whisper.
 * @param {Buffer} audioBuffer - Raw audio bytes
 * @param {string} filename    - Original filename (used for MIME detection)
 * @returns {Promise<string>}  - Transcribed text
 */
async function transcribeAudio(audioBuffer, filename) {
  const FormData = (await import('node-fetch')).default; // node-fetch v3 ships FormData
  // We'll use the global FormData if available (Node 18+), else build multipart manually
...
/**
 * Converts text to speech using Groq's PlayAI TTS endpoint.
...
async function textToSpeech(text) {
  const response = await fetch('https://api.groq.com/openai/v1/audio/speech', {
    ...
    body: JSON.stringify({
      model: 'playai-tts',
      voice: 'Fritz-PlayAI',   // friendly, clear voice
      input: text,
      response_format: 'mp3',
    }),
  });
...
/**
 * Ask Groq AI for a plain-text reply (no markdown) — better for TTS.
 */
async function askGroqForVoice(channelId, userMessage) {
  const history = aiVoiceConversations.get(channelId) || [];
  ...
  const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 300, // keep TTS replies concise
      messages: [
        {
          role: 'system',
          content:
            'You are a friendly, helpful AI voice assistant in a Discord server. ' +
            'Keep replies SHORT (2-4 sentences max) and conversational — they will be read aloud. ' +
            'Do NOT use markdown, bullet points, asterisks, or any special formatting. ' +
            'Speak naturally as if you are talking to a person.',
        },
        ...history,
      ],
    }),
  ...
  history.push({ role: 'assistant', content: reply });
  if (history.length > 20) history.splice(0, history.length - 20);
  aiVoiceConversations.set(channelId, history);
...
client.on('messageCreate', async (message) => {
  ...
  // ── AI VOICE CHANNEL: handle audio attachments ─────────────────────────────
  if (aiVoiceConversations.has(message.channel.id) && !message.content.startsWith('.')) {
    if (message.content.startsWith('✅') || message.content.startsWith('❌')) return;

    const audioAttachment = message.attachments.find(a =>
      a.contentType && (
        a.contentType.startsWith('audio/') ||
        a.name?.match(/\.(mp3|mp4|m4a|wav|ogg|webm|flac|aac)$/i)
      )
    );

    if (audioAttachment) {
      try {
        const typingMsg = await message.channel.send('🎙️ *Transcribing your voice message...*');
        ...
        let transcript;
        try {
          transcript = await transcribeAudio(audioBuffer, audioAttachment.name || 'voice.mp3');
        } catch (e) {
          await typingMsg.delete().catch(() => {});
          return message.reply(`❌ Could not transcribe audio: ${e.message}`);
        }

        if (!transcript) {
          await typingMsg.delete().catch(() => {});
          return message.reply('❌ Could not understand the audio. Please try again and speak clearly.');
        }

        await typingMsg.edit(`🎙️ **You said:** "${transcript}"\n\n⏳ *Generating voice reply...*`);

        // Get AI text reply
        const aiText = await askGroqForVoice(message.channel.id, transcript);

        // Convert to speech
        let ttsBuffer;
        try {
          ttsBuffer = await textToSpeech(aiText);
        } catch (e) {
          // TTS failed — fall back to text reply
          await typingMsg.delete().catch(() => {});
          return message.channel.send(
            `🎙️ **You said:** "${transcript}"\n\n🤖 **AI Reply** *(voice unavailable)*:\n${aiText}`
          );
        }

        await typingMsg.delete().catch(() => {});

        // Send transcript + voice reply
        await message.channel.send({
          content: `🎙️ **You said:** "${transcript}"`,
        });
        await message.channel.send({
          content: '🔊 **AI Voice Reply:**',
          files: [{ attachment: ttsBuffer, name: 'ai-reply.mp3' }],
        });

      } catch (err) {
        console.error('Voice AI error:', err);
        message.reply('❌ Something went wrong processing your voice message. Please try again.');
      }
      return;
    }

    // Text message in voice channel — still respond via text + TTS
    if (message.content.trim()) {
      try {
        await message.channel.sendTyping();
        const aiText = await askGroqForVoice(message.channel.id, message.content);
  ...
  // ── AI CHAT CHANNEL: relay messages to Groq ────────────────────────────────
  if (aiConversations.has(message.channel.id) && !message.content.startsWith('.')) {
    ...
    try {
      ...
      const reply = await askClaude(message.channel.id, message.content, imageUrls);
  ...
  // ── .voiceai ───────────────────────────────────────────────────────────────
  // Owner or admin only: deploys the Voice AI panel embed with a button
  if (command === 'voiceai') {
    ...
    const embed = new EmbedBuilder()
      .setColor(0x57F287)
      .setTitle('🎙️ AI Voice Assistant')
      .setDescription(
        '## Want to chat with AI using your voice?\n\n' +
        'Do you need help with something but don\'t feel like typing it all out? ' +
        '**That\'s totally fine!** Our AI Voice Assistant has got you covered. 🎤\n\n' +
        '**Here\'s how it works:**\n' +
        '> 🟢 Click **Create AI Voice Chat** below\n' +
        '> 🎙️ A private channel opens just for you\n' +
        '> 🔴 Hit the **voice record button** in Discord and send your message\n' +
        '> 🔊 The AI will listen, understand, and **reply back with a real voice message!**\n\n' +
        '**You can also type** if you prefer — the AI will reply with both text and audio.\n\n' +
        '━━━━━━━━━━━━━━━━━━━━━━\n' +
        '🔒 *Your voice chat is completely private — only you and server staff can see it.*\n' +
        '🧠 *The AI remembers your conversation so you can have a natural back-and-forth.*\n' +
        '⚡ *Powered by Groq\'s ultra-fast Whisper & PlayAI voice technology.*'
      )
      .setFooter({ text: 'AI Voice Chat • made by Purpleskeleton__' })
      .setTimestamp();

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId('create_ai_voice_chat')
        .setLabel('🎙️ Create AI Voice Chat')
...
client.on('interactionCreate', async (interaction) => {
  ...
  // ── Create AI Voice Chat button ────────────────────────────────────────────
  if (interaction.isButton() && interaction.customId === 'create_ai_voice_chat') {
    await interaction.deferReply({ ephemeral: true });

    const guild = interaction.guild;
    const user = interaction.user;

    // Check if user already has an open voice chat
    if (aiVoiceChannels.has(user.id)) {
      const existing = guild.channels.cache.get(aiVoiceChannels.get(user.id));
      if (existing) {
        return interaction.editReply(`❌ You already have an open AI Voice Chat: ${existing}\nPlease use that channel or close it first.`);
      } else {
        aiVoiceChannels.delete(user.id);
        aiVoiceConversations.delete(aiVoiceChannels.get(user.id));
      }
    }

    // Find an "AI" or "voice" category if it exists
    const category = guild.channels.cache.find(
      c => c.type === ChannelType.GuildCategory &&
        (c.name.toLowerCase().includes('ai') || c.name.toLowerCase().includes('voice'))
    );
    ...
    try {
      channel = await guild.channels.create({
        name: `🎙️-voice-ai-${user.username.toLowerCase().replace(/[^a-z0-9]/g, '')}`,
        type: ChannelType.GuildText,
        parent: category?.id,
        permissionOverwrites,
        topic: `🎙️ Private AI Voice Chat for ${user.username} (${user.id})`,
      });
    } catch (err) {
      console.error('Failed to create AI voice chat channel:', err);
      return interaction.editReply('❌ Failed to create your AI Voice Chat channel. Please contact a server admin.');
    }

    aiVoiceChannels.set(user.id, channel.id);
    aiVoiceConversations.set(channel.id, []);

    const welcomeEmbed = new EmbedBuilder()
      .setColor(0x57F287)
      .setTitle('🎙️ Your Private AI Voice Chat')
      .setDescription(
        `Welcome <@${user.id}>! 👋\n\n` +
        '**You\'re all set to start chatting with AI using your voice!**\n\n' +
        '**How to send a voice message:**\n' +
        '> 1️⃣ Press and hold the **microphone button** in the message bar\n' +
        '> 2️⃣ Speak your question or request clearly\n' +
        '> 3️⃣ Release to send — the AI will transcribe and reply!\n' +
        '> 🔊 You\'ll receive both a **text reply** and a **voice audio message**\n\n' +
        '**Tips for best results:**\n' +
        '• Speak clearly and at a normal pace\n' +
        '• Keep messages under 2 minutes for fastest responses\n' +
        '• You can also just **type** if you prefer\n' +
        '• The AI remembers your conversation — no need to repeat yourself!\n\n' +
        '━━━━━━━━━━━━━━━━━━━━━━\n' +
        '*When you\'re done, click **Close Voice Chat** below.*\n' +
        '*This channel is private — only you and staff can see it.*'
      )
      .setThumbnail(user.displayAvatarURL())
      .setFooter({ text: 'AI Voice Chat • Powered by Groq Whisper & PlayAI • made by Purpleskeleton__' })
      .setTimestamp();

    const closeRow = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`close_ai_voice_chat_${user.id}`)
        .setLabel('🔒 Close Voice Chat')
        .setStyle(ButtonStyle.Danger)
    );

    await channel.send({ content: `<@${user.id}>`, embeds: [welcomeEmbed], components: [closeRow] });
    await interaction.editReply(`✅ Your private AI Voice Chat has been created: ${channel}`);
    return;
  }

  // ── Close AI Voice Chat button ─────────────────────────────────────────────
  if (interaction.isButton() && interaction.customId.startsWith('close_ai_voice_chat_')) {
    const ownerId = interaction.customId.replace('close_ai_voice_chat_', '');
    ...
    await interaction.reply({ content: '🔒 Closing your AI Voice Chat in 5 seconds...' });

    const closeEmbed = new EmbedBuilder()
      .setColor(0xFF0000)
      .setTitle('🔒 AI Voice Chat Closed')
      .addFields({ name: 'Closed by', value: `@${interaction.user.username}` })
      .setTimestamp();

    await interaction.channel.send({ embeds: [closeEmbed] });

    aiVoiceChannels.delete(ownerId);
    aiVoiceConversations.delete(interaction.channel.id);
  ...
  if (interaction.isButton() && interaction.customId === 'create_ai_chat') {
    ...
    aiConversations.set(channel.id, []);
