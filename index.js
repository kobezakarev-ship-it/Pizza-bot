...
//  LEVELING SYSTEM
// ─────────────────────────────────────────────
const xpData = {};           // xpData[guildId][userId] = { xp, level, lastMessage }
const levelUpChannels = {};  // levelUpChannels[guildId] = channelId

const XP_PER_MESSAGE   = 15;
const XP_COOLDOWN_MS   = 60000; // 1 minute cooldown
...
// Award XP for normal chat (not commands)
...
{ name: ‘XP Set To’, value: `${userData.xp}`, inline: true },
...
// ══════════════════════════════════════════
//  .add_level @user <amount>  — MOD ONLY
// ══════════════════════════════════════════
...
if (isNaN(amount) || amount <= 0) return message.reply(‘❌ Provide a valid positive number of levels.’);
const guildId  = message.guild.id;
ensureUser(guildId, target.id);
const userData = xpData[guildId][target.id];
