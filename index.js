...
try{await target.timeout(60*60*1000,reason);}catch(e){}
...
if(tickets[guildId]?.[userId])return message.reply(`❌ You already have a ticket: <#${tickets[guildId][userId]}>`);
ticketCount++;
const ticketName=`ticket-${String(ticketCount).padStart(4,'0')}`;
try{
const ticketChannel=await message.guild.channels.create({name:ticketName,type:ChannelType.GuildText,permissionOverwrites:[{id:message.guild.id,deny:[PermissionFlagsBits.ViewChannel]},{id:userId,allow:[PermissionFlagsBits.ViewChannel,PermissionFlagsBits.SendMessages]}]});
...
const embed=new EmbedBuilder().setTitle(`🍕 Ticket #${String(ticketCount).padStart(4,'0')}`).setColor(0xFF6600).setThumbnail(message.author.displayAvatarURL()).setDescription(`Hello ${message.author}, welcome to your support ticket!`).addFields({name:'Rules',value:'Be respectful.\nStay on topic.\nDo not ping staff repeatedly.'}).setTimestamp();
...
}catch(e){message.reply('❌ Could not create ticket.');console.error(e);}
}
